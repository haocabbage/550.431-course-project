...
lambda.sg <- apply(parameter.sg[20:51, 1:3], 2, mean)
gamma.sg <- apply(parameter.sg[20:51, 4:6], 2, mean)
m_sg <- gibbs_sampling(seg_crop, GDA.x, lambda.sg, gamma.sg, 300)
par(mar=c(0,0,0,0))
...
training.msg <- BP_alg(seg_crop, GDA.x, lambda.sg, gamma.sg, 1)
...
test.msg <- BP_alg(seg_test_crop, test_crop, lambda.sg, gamma.sg, 5)
...
for (i in 1:length(m_em)) {
  if (m_em[i] == black) {
    ...
    for (j in 1:length(mu)) {
      pr[j] <- dnorm(y[i], mu[j], sigma[j])*phi[j]  
    }
    pr <- pr/sum(pr)
    # sample y_s
